...
beautiful.init("/home/sven/.config/awesome/themes/zenburn/theme.lua")
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "web", "chat", "edit", "stuff1", "stuff2", "stuff3" }, s, layouts[1])
end
-- }}}
...
mymainmenu = awful.menu({ items = { 
                    { "awesome", myawesomemenu, beautiful.awesome_icon },
                                    ...
                                    { "project64", "bash /home/sven/wine_programs/startproj64.sh" },
                                    { "zsnes", "zsnes" },
                                    { "crawl (cdo)", "konsole --profile crawl" },
                                    { "crawl (termcast)", "konsole --profile termcast" },
                                    { "kdevelop", "/home/sven/Projekte/kde/kdevelop/start_kdevelop.sh" },
                                    { "i2c logtool", "python /home/sven/Projekte/radioteleskop/radio_logtool/main.py" }
                                  }
                })
...
function delay(f, timeout_)
    local t = timer({timeout = timeout_ or 0.05})
    t:connect_signal(
       "timeout",
       function()
           t:stop()
...
function focusClientUnderMouse()
    delay(
        function()
...
globalkeys = awful.util.table.join(
    ...
    -- Prompt
    awful.key({  },            "Execute",     function () mypromptbox[mouse.screen]:run() end),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey            }, "c",      function (c) if not awful.rules.match(c, { name = "plasma-desktop" }) then c:kill() end  end),
    ...
    awful.key({ modkey,           }, "n",
        function (c)
            if not awful.rules.match(c, { class = "Plasma" }) then
	        c.minimized = true
	    end
        end),
    awful.key({ modkey,           }, "m",
...
awful.rules.rules = {
    ...
    { rule = { name = "yakuake" }, properties = { floating = true } },
    { rule = { name = "krunner" }, properties = { floating = true } },
...
-- awful.util.spawn_with_shell("kmix; qdbus org.kde.kmix /kmix/KMixWindow org.qtproject.Qt.QWidget.hide")
-- awful.util.spawn_with_shell("nm-applet")
-- awful.util.spawn_with_shell("batti")
-- awful.util.spawn_with_shell("klipper")
-- awful.util.spawn_with_shell("nvidia-settings -l")
awful.util.spawn_with_shell("nvidia-settings")
awful.util.spawn_with_shell("xcompmgr -c -r 10 -l 0 -t 0 -o 0.0 -C")
awful.util.spawn_with_shell('xmodmap -e "clear Lock"')
awful.util.spawn_with_shell('xmodmap -e "keycode 66 = F2"')
